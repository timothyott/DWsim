...
def main(inputFile, runs, thresh, chunky, ss_prob):    
    ...
    num_dw_steps = [[] for i in range(P)]
    num_dw_steps_per_domain = [[] for i in range(P)]
    num_dw_scans = [[] for i in range(P)]
    final_dw_fitness = [[] for i in range(P)]
    norm_dw_fitness = [[] for i in range(P)]
    final_dw_diff = [[] for i in range(P)]
    nonglobal_dw_fit = [[] for i in range(P)]
    dw_global_count = [0 for i in range(P)]
    dw_array = [[float('nan'),float('nan'),float('nan'),[]] for i in range(3)]
    ...
    for land in range(i): 
        ...
        for _ in range(runs):
            ...
            for p in range(P):
                (dw_steps, dw_scans, dw_fit, dw_steps_per_domain)= decision_weaving_threshold(start, N, P, D, NK[land], Power_key, Domain_decision_set, threshold, ss_prob, p)
                dw_array[p][0] = dw_steps
                dw_array[p][1] = dw_scans
                dw_array[p][2] = dw_fit
                dw_array[p][3] = dw_steps_per_domain
            ...
            for p in range(P):
                num_dw_steps[p].append(dw_array[p][0])
                num_dw_steps_per_domain[p].append(dw_array[p][3])
                num_dw_scans[p].append(dw_array[p][1])
                final_dw_fitness[p].append(dw_array[p][2])
                norm_dw_fit = (dw_array[p][2])/max_fit
                norm_dw_fitness[p].append(norm_dw_fit)
                final_dw_diff[p].append(norm_dw_fit - norm_start_fit)
                if dw_array[p][2] == max_fit:
                    dw_global_count[p] += 1
                else:
                    nonglobal_dw_fit[p].append(norm_dw_fit)
    ...
    dw_out = []
    for p in range(P):
        print("Decision Weaving results for " + str(i) + " landscapes starting on domain: " + str(p))
        print_fitness(final_dw_fitness[p], norm_dw_fitness[p], final_dw_diff[p])    
        print_num_steps(num_dw_steps[p])
        print_num_domain_steps(num_dw_steps_per_domain[p])
        print_num_scans(num_dw_scans[p])
        #B&S stats
        print_fitness_peak((dw_global_count[p]/(i*runs)), nonglobal_dw_fit[p])
...
def decision_weaving_threshold(Current_position, N, P, D, NK, Power_key, Domain_decision_set, threshold, ss_prob, p):
    New_position = Current_position.copy()
    #why?
    New_position[0] = abs(Current_position[0] - 1)
    unvisited_policies = np.arange(P)
    ...
    #Need to set the first domain to p, and shuffle the rest
    temp_policies = np.delete(unvisited_policies, p)
    np.random.shuffle(temp_policies)
    unvisited_policies[0]=p
    unvisited_policies[1:]=temp_policies
