...
def main(inputFile):    
    ...
    threshold = int(input("Input the learning threshold: "))
    chunk_key = int(input("Input the type of chunky search (1 = \"chunky\", 2 = \"incremental\", or 3 = \"integrated\"): " ))
    ...
    for land in range(i): 
        ...
        for _ in range(runs):
            ...
            (dw_steps, dw_scans, dw_fit, dw_steps_per_domain) = decision_weaving_threshold(start, N, P, D, NK[land], Power_key, Domain_decision_set, threshold)
            ...
            norm_local_fitness.append(local_fit/max_fit)
            final_local_diff.append(local_fit - start_fit)
            ...
            norm_dw_fitness.append(dw_fit/max_fit)
            final_dw_diff.append(dw_fit - start_fit)

            num_chunky_steps.append(chunky_steps)
            num_chunky_scans.append(chunky_scans)
            final_chunky_fitness.append(chunky_fit)
            norm_chunky_fitness.append(chunky_fit/max_fit)
            final_chunky_diff.append(chunky_fit - start_fit)
            # to recreate the table that B&S have need other variables
            if chunky_fit == max_fit:
                global_count += 1
            else:
                nonglobal_chunky_fit.append(chunky_fit/max_fit)
    ...
    # B&S stats
    print_fitness_peak((global_count/(i*runs)), nonglobal_chunky_fit)
